from quantum_model import quantum_ddos_predict
...
def quantum_view(request):
    if request.method == "POST":
        value = float(request.POST.get("input_value"))
        result = quantum_ddos_predict(value)

        return render(request, "quantum_result.html", {
            "result": result
        })

    return render(request, "quantum_input.html")
